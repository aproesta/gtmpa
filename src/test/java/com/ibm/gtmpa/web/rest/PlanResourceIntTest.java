...
public class PlanResourceIntTest {
	...
	public void createPlan() throws Exception {
		...
		restPlanMockMvc.perform(post("/api/plans").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(plan))).andExpect(status().isCreated());
		...
		assertThat(testPlan.getStatus()).isEqualTo(Plan.START_STATUS);
	}
	...
	public void checkSolutionNameIsRequired() throws Exception {
		...
		restPlanMockMvc.perform(post("/api/plans").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(plan))).andExpect(status().isBadRequest());
	...
	public void getAllPlans() throws Exception {
		...
		// Get all the plans
		restPlanMockMvc.perform(get("/api/plans")).andExpect(status().isOk())
	...
	public void getPlan() throws Exception {
		...
		// Get the plan
		restPlanMockMvc.perform(get("/api/plans/{id}", plan.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$.status").value(Plan.START_STATUS.toString()))
	...
	public void updatePlan() throws Exception {
		...
		restPlanMockMvc.perform(put("/api/plans").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(plan))).andExpect(status().isOk());
	...
	public void deletePlan() throws Exception {
		...
		// Get the plan
		restPlanMockMvc.perform(delete("/api/plans/{id}", plan.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
